...
#else
	if (get_n_rand32((ULONG32 *)rand, 2, 20) == 2)
	{
		return 1;
	}
#endif
...
# else
	if (get_n_seed32((ULONG32 *)seed, 2, 2 * retries) == 2)
	{
		return 1;
	}
# endif
...
ULONG DRNG::seed_from_rdrand(void *buf, ULONG n)
{
	...
	if (get_n_rand64((ULONG64 *)key, 2, 20) != 2)
	{
		return 0;
	}

	// Set up encryption
	NTSTATUS status = BCryptOpenAlgorithmProvider(&halgo, BCRYPT_AES_ALGORITHM, NULL, 0);
